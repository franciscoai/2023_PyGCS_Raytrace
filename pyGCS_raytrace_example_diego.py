...
for sat in range(n_sat):
    ...
    if otype=="fits":
        ...
        cme.writeto(OPATH +'/{:08.2f}_{:08.3f}_{:08.3f}_{:08.3f}_{:08.3f}_{:08.3f}_sat{}_btot.fits'.format(
            gcs_par[0], gcs_par[1], gcs_par[2], gcs_par[3], gcs_par[4], gcs_par[5], sat+1), overwrite=True)
    elif otype =="png":
    #full image
        m = np.mean(btot)
        sd = np.std(btot)
